import type { PersonalInfo } from "@/types";

export const personalInfo: PersonalInfo = {
  name: "Piotr Romanczuk",
  title: "Software Engineer",
  email: "p.romanczuk@gmail.com",
  phone: "+48 123 456 789",
  location: "Warsaw, PL",
  tagline:
    "Building digital experiences with code and coffee. Focusing on clean aesthetics and functional minimalism.",
  bio: "I'm a software developer based in Poland, specializing in full-stack web development. I believe that good design is as little design as possible. My goal is to create interfaces that feel natural and code that is maintainable.",
  techSummary:
    "I mostly work with TypeScript, React, Next.js, and Tailwind. Sometimes Supabase, sometimes plain Node.",
  toolkit: ["React", "TypeScript", "Node.js", "Tailwind", "Python"],
  github: "https://github.com/PiotrRomanczuk",
  linkedin: "https://linkedin.com/in/piotr-romanczuk",
  twitter: "#",
};
